...
export default function Header({ user, cartItemCount = 0, initialSearch = '' }: HeaderProps) {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (!dropdownRef.current?.contains(e.target as Node) && !searchRef.current?.contains(e.target as Node)) {
        setTimeout(() => { setDropdownOpen(false); setShowSuggestions(false); }, 150);
      }
    };
  ...
  return (
    <header className="sticky top-0 z-10 flex flex-col md:flex-row items-center justify-between gap-3 md:gap-0 bg-blue-600 px-4 md:px-6 py-3 text-white shadow-md">
      {/* Logo */}
      ...
      <div ref={searchRef} className="relative w-full md:mx-6 md:flex-1 md:max-w-xl">
        <form onSubmit={handleSubmit} className="relative w-full mt-2 md:mt-0">
        ...
        {/* Suggestion Box */}
        {showSuggestions && (
          <div className="absolute left-0 right-0 z-50 mt-1 max-h-80 overflow-y-auto rounded-md border border-gray-200 bg-white shadow-lg">
            ...
            {dropdownOpen && (
              <div className="absolute right-0 z-20 mt-2 w-40 rounded-md border border-gray-200 bg-white shadow-lg">
                ...
                <button
                  ...
                  onClick={() => { router.post(route('logout')); setDropdownOpen(false); }}
                >
                  Logout
                </button>
              </div>
